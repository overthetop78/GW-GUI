...
- ajouter dans Conversion un parcours explicite de déprotection qui ne modifie jamais la source : normalisation directe lorsque supprimer les marques de protection suffit, et convertisseur dédié par famille lorsque le chargeur doit être adapté. RWTS18 exige un traitement dédié, car ses 18 pages (4 608 octets) par piste ne tiennent pas sans perte dans les 16 secteurs (4 096 octets) d'une piste DOS 3.3 ; une sortie ne doit être proposée comme amorçable qu'après validation du chargeur transformé ;
