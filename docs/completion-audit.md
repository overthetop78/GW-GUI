...
- `scripts/test-scp-corpus.ps1` télécharge trois captures physiques Greaseweazle publiées sur Internet Archive : les OS-9/CD-i confirmées fonctionnelles `os9sys.scp` et `pubsoft1.scp`, plus la démonstration AMOS Professional marquée domaine public. Il vérifie taille, MD5 et SHA-1, puis exerce le lecteur SCP complet, checksums, pistes/faces/révolutions, auto-détection `iso.*`/`amiga.mfm` et rendu WPF/Skia non vide. Les quelque 105 Mo sont supprimés après le test et ne sont pas redistribués.
...
- Le visualiseur SCP lit et rend le conteneur, les deux faces, pistes, structures, anomalies, zooms et inspecteur. Les familles déjà décodées ont des tests synthétiques; ISO possède deux captures physiques publiques et Amiga MFM une capture domaine public vérifiée. La couverture exhaustive des autres analyseurs HxC annoncée dans le plan n’est pas encore démontrée par un corpus physique libre.
