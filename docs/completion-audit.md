...
- `scripts/test-scp-corpus.ps1` télécharge deux captures physiques Greaseweazle OS-9/CD-i confirmées fonctionnelles et publiées sur Internet Archive (`os9sys.scp`, `pubsoft1.scp`), vérifie taille, MD5 et SHA-1, puis exerce le lecteur SCP complet, checksums, pistes/faces/révolutions, auto-détection de la famille ISO et rendu WPF/Skia non vide. Les fichiers sont supprimés après le test et ne sont pas redistribués.
...
- Le visualiseur SCP lit et rend le conteneur, les deux faces, pistes, structures, anomalies, zooms et inspecteur. Les familles déjà décodées ont des tests synthétiques et la famille ISO possède désormais deux captures physiques publiques vérifiées; la couverture exhaustive des autres analyseurs HxC annoncée dans le plan n’est pas encore démontrée par un corpus physique libre.
