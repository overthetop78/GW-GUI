...
## 15. Inventaire et journal de migration

Cette section est le registre durable de la refonte. Avant toute modification d'un lot de code, l'état réel des fichiers concernés y est consigné après comparaison de `GWGUI.App`, `GWGUI.Emulation`, `GWGUI.Emulation.Amiga` et `GWGUI.Emulation.Atari`. Après le traitement du lot, cette section doit indiquer les fichiers travaillés, ce qui a été fait et pourquoi cela a été fait.

Une entrée ne doit jamais prétendre qu'un fichier a été vérifié s'il n'a pas été lu. L'inventaire peut être complété progressivement, mais le code d'une responsabilité ne doit pas être modifié avant que ses équivalents connus aient été examinés et consignés ici.

### 15.1 État initial vérifié avant modification

#### Raccordements des projets

- `src/GWGUI.App/GWGUI.App.csproj` référence directement `GWGUI.Emulation`, `GWGUI.Emulation.Amiga` et `GWGUI.Emulation.Atari`. Cela correspond à l'architecture retenue sans découverte dynamique de DLL.
- `src/GWGUI.App/Controls/OptionsEmulationSection.cs` raccorde actuellement lui-même les paramètres Amiga et Atari. La partie Amiga est largement construite directement dans ce fichier, tandis que la partie Atari est fournie par `AtariConfigurationCatalogSection`. Les deux familles n'utilisent donc pas la même construction graphique.
- `src/GWGUI.App/Controls/EmulationSection.cs` raccorde actuellement l'exécution. Il distingue explicitement les configurations Amiga et Atari, construit un `AmigaEngine` ou un `AtariEngine`, crée la machine, puis confie cette instance au contrôleur graphique correspondant.

#### Contrats communs actuels

- `src/GWGUI.Emulation/EmulationContracts.cs` mélange actuellement dans un seul fichier l'état de machine, les interfaces de machine et d'Engine, la vidéo, l'audio, les médias et les entrées. Ce fichier ne respecte pas le découpage retenu entre `Interfaces`, `Enums` et `Contracts`.
- `IEmulatedMachine` contient déjà le cycle de vie commun : démarrage, pause, reprise, réinitialisation logicielle, réinitialisation matérielle et arrêt.
- La représentation actuelle des médias utilise un enum fermé `EmulationMediaSlot` avec `Floppy0` à `Floppy3`, un seul disque dur, un seul CD, une cartouche et une cassette. Elle ne permet pas le nombre arbitraire de lecteurs décidé dans ce document.
- `EmulationMediaType` contient actuellement `Directory`, alors que le document interdit de traiter un dossier comme un périphérique ou un support montable commun.

#### Interfaces de machines actuelles

- `src/GWGUI.Emulation.Amiga/IAmigaMachine.cs` et `src/GWGUI.Emulation.Atari/IAtariMachine.cs` étendent toutes deux `IEmulatedMachine` et exposent des responsabilités parallèles : configuration, vidéo, audio, options d'exécution, diagnostics, LED, informations d'émulateur, entrées, médias et sauvegardes d'état.
- Elles divergent dans leurs types et signatures : Amiga expose notamment le nombre et l'index de disque, tandis qu'Atari expose `AtariRuntimeStatus`, le volume audio, la sélection du périphérique d'un port et des médias Atari spécifiques.
- Les deux interfaces exposent encore `Fault` et `Diagnostics`, alors que le document retient des journaux techniques propres aux bibliothèques et un contrat commun de messages utilisateur, sans conserver ces deux notions dans le contrat initial cible.
- Les deux interfaces sont encore des contrats familiaux directement connus d'App. Leur comparaison confirme qu'il faut extraire les capacités réellement communes avant de supprimer les dépendances graphiques aux interfaces familiales.

#### Engines et instances actuels

- `src/GWGUI.Emulation.Amiga/AmigaEngine.cs` et `src/GWGUI.Emulation.Atari/AtariEngine.cs` ont la même responsabilité générale : conserver les dépendances nécessaires à la création, créer un identifiant d'instance, choisir ou construire l'hôte de l'émulateur, créer le dossier de session, puis retourner une nouvelle machine.
- Les deux Engines implémentent `IEmulationEngine<TConfiguration>`, mais App les recrée actuellement lors de chaque ouverture de machine dans `EmulationSection`. Cela ne correspond pas encore au choix d'un Engine de famille conservé et de Factories internes par émulateur.
- Aucun des deux Engines ne gère les commandes en direct après la création. Ces commandes sont envoyées à `AmigaMachine` ou `AtariMachine` par les contrôleurs App, ce qui correspond déjà au principe retenu.

#### Construction graphique actuelle dans App

- `src/GWGUI.App/Controls/OptionsEmulationSection.cs` contient 1 925 lignes et plusieurs responsabilités Amiga : création des contrôles, disposition des sous-onglets, chargement et construction des configurations, analyse et sélection des ROM, stockage, entrées et catalogue commun des configurations. Il s'agit d'un fichier fourre-tout à découper.
- La configuration Atari est répartie dans de nombreux fichiers `Atari...Section`, `Atari...Functions`, `Atari...Constants` et objets `...View` dans App et dans `GWGUI.Emulation.Atari`. Cette organisation est beaucoup plus fragmentée que la partie Amiga et doit être comparée responsabilité par responsabilité, sans reproduire automatiquement ce découpage.
- `src/GWGUI.App/Controls/AmigaMachineController.cs` et `src/GWGUI.App/Controls/AtariMachineController.cs` sont deux contrôleurs graphiques familiaux de 1 096 et 814 lignes. Ils partagent des responsabilités de vue de machine, commandes, vidéo, audio, entrées, médias et états sauvegardés. Leur contenu doit être comparé avec `MachineView` et les contrôles communs existants avant toute fusion.
- App contient encore des services familiaux `AmigaCoreProvider`, `AtariCoreProvider` et `AmigaRuntimeMedia`. Ils devront être examinés avec les services d'installation et de médias des bibliothèques avant déplacement ; aucun fichier entier ne sera déplacé aveuglément.

#### Différences structurelles déjà confirmées

- La partie Amiga utilise directement `AmigaModelCatalog.All` dans un `ComboBox` créé dans `OptionsEmulationSection`.
- La partie Atari utilise un contrôle graphique séparé et ses propres fonctions de catalogue. Le sélecteur de machine n'est donc pas encore une construction commune alimentée de la même façon par `AmigaModelCatalog.All` et `AtariModelCatalog.All`.
- Les options natives Amiga sont encore largement lues et écrites dans App au moyen de clés `puae_...`. Cette traduction entre options communes et options de l'émulateur devra quitter App et être fournie par `GWGUI.Emulation.Amiga`.
- Plusieurs règles Atari sont déjà dans `GWGUI.Emulation.Atari`, mais la bibliothèque contient de très nombreux fichiers minuscules séparant systématiquement constantes, contrats, erreurs et fonctions. Le découpage cible devra respecter une responsabilité claire sans conserver cette fragmentation lorsqu'elle n'apporte rien.

### 15.2 Lots de migration

Chaque lot est ajouté ici avant son démarrage avec les fichiers examinés, puis complété après modification avec les fichiers travaillés, le changement effectué et sa justification.

#### Lot 1 — Découpage des contrats communs existants

État : réalisé, vérifié structurellement sans compilation.

Fichiers examinés avant modification :

- `src/GWGUI.Emulation/EmulationContracts.cs` ;
- `src/GWGUI.Emulation.Amiga/IAmigaMachine.cs` ;
- `src/GWGUI.Emulation.Atari/IAtariMachine.cs` ;
- `src/GWGUI.Emulation.Amiga/AmigaEngine.cs` ;
- `src/GWGUI.Emulation.Atari/AtariEngine.cs` ;
- leurs références trouvées dans `GWGUI.App`, `GWGUI.Emulation.Amiga` et `GWGUI.Emulation.Atari`.

But du lot : déplacer chaque interface, enum et contrat actuellement regroupé dans `EmulationContracts.cs` vers le dossier et le fichier correspondant à sa responsabilité, sans modifier ses noms, ses valeurs, ses signatures ni son comportement. Les changements fonctionnels des médias et la composition des interfaces de capacités seront traités dans des lots séparés après inventaire de leurs consommateurs.

Travail réalisé :

- suppression de `src/GWGUI.Emulation/EmulationContracts.cs` après répartition complète de son contenu ;
- création dans `Interfaces` de `IEmulatedMachine.cs`, `IEmulationEngine.cs` et `IAudioOutput.cs` ;
- création dans `Enums` de `EmulationMachineState.cs`, `EmulationPixelFormat.cs`, `EmulationVideoRenderer.cs`, `EmulationMediaSlot.cs`, `EmulationMediaType.cs` et `EmulationKey.cs` ;
- création dans `Contracts` de `VideoFrame.cs`, `AudioChunk.cs`, `EmulationMedia.cs`, `EmulationPointerState.cs`, `EmulationControllerState.cs` et `EmulationInputSnapshot.cs`.

Pourquoi : le fichier supprimé mélangeait quinze types de responsabilités différentes. Le nouveau rangement applique les sections 9 et 10 sans modifier le contrat consommé par App, Amiga ou Atari. Une vérification par recherche a confirmé qu'il reste exactement une déclaration de chacun des quinze types déplacés.

#### Lot 2 — Type commun des sous-onglets de machine

État : réalisé, vérifié structurellement sans compilation.

Fichiers examinés avant modification :

- `src/GWGUI.App/Controls/EmulationMachineTabContracts.cs` ;
- `src/GWGUI.App/Controls/EmulationMachineTabs.cs` ;
- `src/GWGUI.App/Controls/OptionsEmulationSection.cs` ;
- `src/GWGUI.App/Controls/AtariHardwareSettingsSection.cs`.

Constat : `EmulationMachineTabContracts.cs` mélange un enum et un record. L'enum commun porte le nom `EmulationMachineTabKind`, interdit par la règle de nommage, et il est utilisé de la même façon par les constructions Amiga et Atari.

But du lot : déplacer l'enum dans `Enums`, le record dans `Contracts`, renommer uniquement l'enum en `EmulationMachineTab` et actualiser ses références sans modifier l'ordre, les onglets, les ressources, les icônes, la visibilité ni les contenus.

Travail réalisé :

- suppression de `src/GWGUI.App/Controls/EmulationMachineTabContracts.cs` ;
- création de `src/GWGUI.App/Enums/EmulationMachineTab.cs` pour l'enum commun ;
- création de `src/GWGUI.App/Contracts/EmulationMachineTabDefinition.cs` pour la définition graphique d'un onglet ;
