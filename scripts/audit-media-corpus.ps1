...
if (-not $Restart -and (Test-Path -LiteralPath $checkpointPath)) {
    ...
    $sameRoot = $checkpoint.root -and ([string]$checkpoint.root -ieq $resolvedRoot)
    if ($sameRoot -and $checkpoint.status -eq 'failed' -and $checkpoint.failedPath) {
    ...
    } elseif ($sameRoot -and ($checkpoint.nextIndex -is [long] -or $checkpoint.nextIndex -is [int])) {
        $startIndex = [int]$checkpoint.nextIndex
    } elseif ($StartAt) {
        $resolvedStart = [System.IO.Path]::GetFullPath($StartAt)
        while ($startIndex -lt $candidates.Count -and [string]::Compare($candidates[$startIndex].FullName, $resolvedStart, $true) -lt 0) {
            $startIndex++
        }
    }
} elseif ($StartAt) {
