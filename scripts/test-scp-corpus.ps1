...
$captures = @(
    [pscustomobject]@{ Identifier='os9sys'; FileName='os9sys.scp'; DecoderPrefix='iso.'; Size=39322724; Md5='63c931f79f33398ec4026bc1f5109cdb'; Sha1='3f20aeb3ac0b7328eb233b76267527d23b790ea3' },
    [pscustomobject]@{ Identifier='pubsoft1'; FileName='pubsoft1.scp'; DecoderPrefix='iso.'; Size=38742166; Md5='6d7fccfa3399c6f06e20d269d707c4f0'; Sha1='d9a6ed364506a85579281d7d416bb6bc569e6c97' },
    [pscustomobject]@{ Identifier='amiga-amos-professional-demo-disc'; FileName='amiga amos professional demo disc.scp'; DecoderPrefix='amiga.mfm'; Size=27447022; Md5='e5cab62655cb779cb70c91bde6a0a21c'; Sha1='43c8fe6c75a50c964c3f040995608bc810f1364d' }
)
...
try {
    ...
    $downloaded = foreach ($capture in $captures) {
        ...
        [pscustomobject]@{ Identifier=$capture.Identifier; DecoderPrefix=$capture.DecoderPrefix; Path=$path; Size=$file.Length; Md5=$md5; Sha1=$sha1 }
    }

    $env:GWGUI_REAL_SCP_CORPUS = [string]::Join([IO.Path]::PathSeparator, @($downloaded | ForEach-Object { "$($_.DecoderPrefix)|$($_.Path)" }))
