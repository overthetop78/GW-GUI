...
def insert(path: Path, key: str, value: str) -> None:
    text = path.read_text(encoding="utf-8")
    if f'name="{key}"' in text:
        return
    newline = "\r\n" if "\r\n" in text else "\n"
    entry = f'  <data name="{escape(key)}"><value>{escape(value)}</value></data>{newline}'
...
def main() -> None:
    ...
    insert(root / args.resource, args.key, args.english)
    insert(root / "en-US" / args.resource, args.key, args.english)
    ...
    for culture, language_code in LANGUAGE_CODES.items():
        ...
        insert(root / culture / args.resource, args.key, value)
