    ...
    CommodoreDosMigrationService commodore)
{
    ...
    {
        var target = FileSystemMigrationTargetCatalog.Get(targetFormatId);
        if (target.FileSystemId.StartsWith(FileSystemIds.AmigaDos, StringComparison.OrdinalIgnoreCase)) return FileSystemMigrationCapabilityCatalog.ForAmigaDos(AmigaDosVariant.Ffs, targetFormatId);
        if (target.FileSystemId.Equals(FileSystemIds.Fat12, StringComparison.OrdinalIgnoreCase)) return FileSystemMigrationCapabilityCatalog.ForFat12(targetFormatId);
        if (target.FileSystemId.Equals(FileSystemIds.AppleDos, StringComparison.OrdinalIgnoreCase)) return FileSystemMigrationCapabilityCatalog.ForAppleDos(targetFormatId);
        if (target.FileSystemId.Equals(FileSystemIds.ProDos, StringComparison.OrdinalIgnoreCase) || target.FileSystemId.Equals(FileSystemIds.Sos, StringComparison.OrdinalIgnoreCase)) return FileSystemMigrationCapabilityCatalog.ForProDos(targetFormatId);
