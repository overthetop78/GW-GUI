...
{
    public MediaSectorWritePlan Create(MigrationPlan plan, Fat12TargetGeometry geometry)
    {
        ArgumentNullException.ThrowIfNull(plan);
        if (geometry.SectorSize != FatBootSectorLayout.SectorSize || geometry.Cylinders <= 0 || geometry.Heads <= 0 || geometry.SectorsPerTrack <= 0) throw Fat12VolumeWriterExceptions.UnsupportedGeometry(geometry.FormatId);
    ...
    private sealed class Builder(MigrationPlan plan, Fat12TargetGeometry geometry)
    {
        ...
        public MediaSectorWritePlan Build()
        {
            ...
            var blocks = Enumerable.Range(0, geometry.TotalSectors).Select(logicalBlock =>
            {
                ...
                return new MediaSectorWriteBlock(logicalBlock, new MediaSectorAddress(track / geometry.Heads, track % geometry.Heads, logicalBlock % geometry.SectorsPerTrack + FatBootSectorLayout.FirstSectorNumber), _sectors[logicalBlock]);
            });
        ...
        private void WriteBootSector()
        {
            ...
            boot[1] = 0x3c;
            boot[2] = 0x90;
            var oem = geometry.IsMsx ? "MSX     " : "GWGUI   ";
