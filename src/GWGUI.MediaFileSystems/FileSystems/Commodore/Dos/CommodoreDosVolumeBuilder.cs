...
internal sealed class CommodoreDosVolumeBuilder(MigrationPlan plan, CommodoreDosWritableGeometry geometry, CommodoreDosWritePolicy policy)
{
    ...
    public MediaSectorWritePlan Build()
    {
        ...
        WriteSystemSectors();
        var blocks = _sectors.Select((data, logical) => new MediaSectorWriteBlock(logical, geometry.CreateAddress(logical), data));
    ...
    {
        if (geometry.FormatId == MediaImageFormatIds.Commodore1581)
        {
        ...
        if (geometry.FormatId == MediaImageFormatIds.Commodore1571) Reserve(Commodore1541DosLayout.HeaderTrack + geometry.Cylinders, Commodore1541DosLayout.HeaderSector);
    }
    ...
    private void AllocateDirectorySectors()
    {
        ...
        {
            var preferredSector = (geometry.FormatId == MediaImageFormatIds.Commodore1581 ? Commodore1581DosLayout.DirectorySector : Commodore1541DosLayout.DirectorySector) + index;
            var preferredTrack = geometry.FormatId == MediaImageFormatIds.Commodore1581 ? Commodore1581DosLayout.HeaderTrack : Commodore1541DosLayout.HeaderTrack;
    ...
    {
        if (geometry.FormatId == MediaImageFormatIds.Commodore1581) Write1581SystemSectors();
    ...
    private void Write1541SystemSectors()
    {
        ...
        if (geometry.FormatId == MediaImageFormatIds.Commodore1571)
        {
    ...
    private void Write1541Bam(Span<byte> bam, int side)
    {
        for (var track = 1; track <= geometry.Cylinders; track++)
        {
            ...
            var sectorCount = Commodore1541Geometry.SectorsPerTrack(track);
    ...
    {
        if (count == 0) return [];
        var center = geometry.FormatId == MediaImageFormatIds.Commodore1581 ? Commodore1581DosLayout.HeaderTrack : Commodore1541DosLayout.HeaderTrack;
