...
public sealed class ProDosVolumeWriter
{
    ...
    public MediaSectorWritePlan Create(MigrationPlan plan, MediaSectorGeometry geometry, IReadOnlyList<MediaSectorAddress> addresses)
    {
        ArgumentNullException.ThrowIfNull(plan);
        ArgumentNullException.ThrowIfNull(geometry);
        ArgumentNullException.ThrowIfNull(addresses);
        if (geometry.FormatId is not (MediaImageFormatIds.AppleIIProDos or MediaImageFormatIds.AppleIIProDos140 or MediaImageFormatIds.AppleIIProDos800))
            throw ProDosVolumeWriterExceptions.UnsupportedFormat(geometry.FormatId);
        if (geometry.SectorSize != ProDosFileSystemLayout.BlockSize || geometry.BlockCount <= 0 ||
            geometry.Cylinders <= 0 || geometry.Heads <= 0 || geometry.SectorsPerTrack <= 0 ||
            addresses.Count != geometry.BlockCount)
            throw new InvalidDataException("The target ProDOS geometry or sector address map is invalid.");
        var state = new BuildState(geometry.BlockCount);
        state.ReserveSystemBlocks();
        WriteRootDirectory(state, plan);
        WriteBitmap(state);
        return CreateImage(geometry, addresses, state.Blocks);
    }
    ...
    private static MediaSectorWritePlan CreateImage(MediaSectorGeometry geometry, IReadOnlyList<MediaSectorAddress> addresses, byte[][] data)
    {
        var blocks = data.Select((block, logical) => new MediaSectorWriteBlock(logical, addresses[logical], block));
        return new(geometry.FormatId, ProDosFileSystemLayout.BlockSize, geometry.Cylinders, geometry.Heads, geometry.SectorsPerTrack, blocks, logicalBlockCount: geometry.BlockCount);
    }
