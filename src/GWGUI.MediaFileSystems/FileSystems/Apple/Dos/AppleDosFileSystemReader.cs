...
public sealed class AppleDosFileSystemReader : IFileSystemReader
{
    ...
    /// <inheritdoc />
    public IReadOnlySet<string> CatalogFormatIds { get; } = new[] { MediaImageFormatIds.AppleIIDos32, MediaImageFormatIds.AppleIIDos33, MediaImageFormatIds.AppleIIAppleDos140 }.ToFrozenSet(StringComparer.OrdinalIgnoreCase);
    /// <inheritdoc />
    public bool CanRead(IMediaSectorImage image) => AppleDosVtocReader.TryRead(image, out _);
    ...
    private static void ReadCatalogSector(IMediaSectorImage image, byte[] bytes, ICollection<FileSystemEntry> entries, ICollection<string> warnings)
    {
