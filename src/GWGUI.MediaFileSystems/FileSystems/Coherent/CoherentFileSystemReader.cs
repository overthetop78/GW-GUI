...
namespace GWGUI.MediaFileSystems.FileSystems.Coherent;
...
public sealed class CoherentFileSystemReader : IFileSystemReader
{
    ...
    public IReadOnlySet<string> CatalogFormatIds { get; } = new[] { MediaImageFormatIds.Commodore900Coherent }.ToFrozenSet(StringComparer.OrdinalIgnoreCase);

    /// <summary>Indique si l'image utilise le format et la taille de bloc COHERENT attendus.</summary>
    public bool CanRead(IMediaSectorImage image) => CatalogFormatIds.Contains(image.FormatId) && image.BlockSize == CoherentFileSystemLayout.BlockSize;
