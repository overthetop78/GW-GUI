...
public static class AcornAdfsFileReader
{
    ...
    public static IReadOnlyList<byte>? Read(IMediaSectorImage image, int address, uint length, IFileCoreAddressResolver resolver, string name, List<string> warnings, ref bool metadataValid)
    {
        if (length == 0) return [];
