...
public static class CoreHost
{
                ...
                switch (command)
                {
                    case HostCommand.Initialize:
                        ...
                        var configuration = JsonSerializer.Deserialize<MachineConfiguration>(reader.ReadString(), CoreHostProtocol.JsonOptions)
                            ?? throw new InvalidDataException(Caprice32Exceptions.HostConfigurationInvalid());
                    ...
                    case HostCommand.SoftReset: EnsureCore(core).SoftReset(); WriteSuccess(writer); break;
                    ...
                    default: throw new InvalidDataException(Caprice32Exceptions.UnknownHostCommand((byte)command));
                }
            }
    ...
    private static ExternalCore EnsureCore(ExternalCore? core) => core ?? throw new InvalidOperationException(Caprice32Exceptions.HostNotInitialized());
