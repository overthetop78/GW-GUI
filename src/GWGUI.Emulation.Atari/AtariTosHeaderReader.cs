...
internal static class AtariTosHeaderReader
{
    ...
    internal static async Task<AtariTosHeader?> ReadAsync(string path, CancellationToken cancellationToken)
    {
        ...
        var variant = text.Contains("EmuTOS", StringComparison.OrdinalIgnoreCase)
            ? AtariTosVariant.EmuTos
            : text.Contains("KAOS", StringComparison.OrdinalIgnoreCase)
                ? AtariTosVariant.KaosTos
                : AtariTosVariant.Atari;
        var version = variant switch
        {
            AtariTosVariant.EmuTos => FindVersion(text, "EmuTOS", headerVersion),
            AtariTosVariant.KaosTos => FindVersion(text, "KAOS - TOS", headerVersion),
            _ => headerVersion
        };
    ...
    private static string FindVersion(string text, string product, string fallback)
    {
        var productVersion = System.Text.RegularExpressions.Regex.Match(text,
            System.Text.RegularExpressions.Regex.Escape(product) + @"[^0-9]{0,16}(?<version>[0-9]+(?:\.[0-9]+){1,3})",
            System.Text.RegularExpressions.RegexOptions.IgnoreCase);
        if (productVersion.Success) return productVersion.Groups["version"].Value;

        var standaloneVersion = System.Text.RegularExpressions.Regex.Match(text,
            @"(?<![0-9])(?<version>0\.[0-9]+(?:\.[0-9]+){0,2})(?![0-9])");
        return standaloneVersion.Success ? standaloneVersion.Groups["version"].Value : fallback;
    }
