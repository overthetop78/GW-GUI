...
internal static class CoreDiagnosticFunctions
{
    ...
    internal static IReadOnlyList<string> ReadExports(string path)
    {
        try
        {
            ...
            if (directory.RelativeVirtualAddress == BufferConstants.FirstBufferIndex
            ...
            for (var index = BufferConstants.FirstBufferIndex; index < count; index++)
            {
    ...
    private static string ReadNullTerminatedString(BinaryReader reader)
    {
        ...
        while (bytes.Count < CoreReleaseConstants.MaximumExportNameLength)
        {
            ...
            if (value == BufferConstants.FirstBufferIndex) break;
            bytes.Add(value);
        }
