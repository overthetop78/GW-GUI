...
internal static class EnvironmentFunctions
{
    ...
    internal static IReadOnlyList<InputDescriptor> CopyInputDescriptors(nint data)
    {
        ...
        for (var index = BufferConstants.FirstCollectionIndex;
    ...
    internal static IReadOnlyList<ControllerPort> CopyControllerPorts(nint data)
    {
        ...
        for (var portIndex = BufferConstants.FirstCollectionIndex;
             portIndex < EnvironmentConstants.MaximumControllerPortCount; portIndex++)
        {
            ...
            if (port.Types == nint.Zero
                || port.Count == ExternalCoreInteropConstants.EmptyNativeCollectionCount) return ports;
            var devices = new List<ControllerDevice>();
            var count = Math.Min(port.Count, (uint)EnvironmentConstants.MaximumControllerTypeCount);
            for (var deviceIndex = BufferConstants.FirstCollectionIndex; deviceIndex < count; deviceIndex++)
            {
    ...
    internal static IReadOnlyList<MemoryDescriptor> CopyMemoryMap(nint data)
    {
        ...
        if (map.Descriptors == nint.Zero
            || map.Count == ExternalCoreInteropConstants.EmptyNativeCollectionCount) return [];
        ...
        for (var index = BufferConstants.FirstCollectionIndex; index < count; index++)
        {
    ...
    internal static string CopyNativeLogTemplate(nint format)
    {
        ...
        for (var index = BufferConstants.FirstCollectionIndex; index < source.Length; index++)
        {
