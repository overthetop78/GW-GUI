    ...
    internal static AtariStateFile Read(string path)
    {
        try
        {
            ...
            if (!magic.SequenceEqual(AtariStateConstants.Magic))
                throw AtariSavedStateFunctions.Invalid(AtariErrorCode.StateInvalid, AtariStateConstants.InvalidMagicError);
            ...
            if (header is null || !AtariSavedStateFunctions.IsHeaderValid(header))
                throw AtariSavedStateFunctions.Invalid(AtariErrorCode.StateInvalid, AtariStateConstants.InvalidHeaderError);
