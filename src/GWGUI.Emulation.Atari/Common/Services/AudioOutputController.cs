...
internal sealed class AudioOutputController : IDisposable
{
    ...
    private float _volume = AudioConstants.DefaultVolume;
    private bool _muted;
    ...
    internal AudioOutputController(IAudioOutput? output = null, Func<IAudioOutput?>? factory = null)
    {
        _output = output;
        _factory = factory;
    }
    ...
    internal void Write(AudioChunk chunk)
    {
        lock (_gate)
        {
            ...
            var samples = AudioOutputFunctions.ApplyVolume(chunk.InterleavedStereo.Span, _volume,
                ref _volumeBuffer);
    ...
    {
        lock (_gate) _volume = AudioOutputFunctions.NormalizeVolume(volume);
    }
    ...
    private bool EnsureStarted()
    {
        if (_stopped || _paused || _muted || _unavailable
            || _sampleRate <= MachineConstants.InvalidSampleRate) return false;
