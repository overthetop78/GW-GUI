...
internal static partial class InputSettingsFunctions
{
        ...
        PeripheralCategory peripheral)
    {
        ...
        if (model == MachineModel.Atari5200 && peripheral is PeripheralCategory.AnalogJoystick
            ...
            return ControllerConstants.DirectionActions.Concat(ControllerConstants.DualFireActions)
                .Concat(ControllerConstants.KeypadActions).Distinct(StringComparer.Ordinal);
        if (peripheral == PeripheralCategory.NumericKeypad) return ControllerConstants.KeypadActions;
        if (peripheral is PeripheralCategory.Paddle or PeripheralCategory.DrivingController
            or PeripheralCategory.LightGun) return ControllerConstants.SingleFireActions;
        if (peripheral == PeripheralCategory.BoosterGrip)
            return ControllerConstants.DirectionActions.Concat([InputSettingsFunctionsConstants.Fire1, InputSettingsFunctionsConstants.Fire2, InputSettingsFunctionsConstants.Turbo]);
        if (peripheral == PeripheralCategory.Joy2BPlus)
            return ControllerConstants.DirectionActions.Concat(ControllerConstants.DualFireActions);
        if (peripheral == PeripheralCategory.GenesisController)
            return ControllerConstants.DirectionActions.Concat(ControllerConstants.SingleFireActions);
        if (model is MachineModel.Jaguar or MachineModel.JaguarCd)
            return ControllerConstants.DirectionActions.Concat(ControllerConstants.JaguarActions);
        if (model == MachineModel.Lynx)
            return ControllerConstants.DirectionActions.Concat(ControllerConstants.LynxActions);
        if (model is MachineModel.Atari5200 or MachineModel.Atari7800)
            return ControllerConstants.DirectionActions.Concat(ControllerConstants.DualFireActions);
        if (CompatibilityCatalog.Get(model).Core == Emulator.Hatari)
            return ControllerConstants.DirectionActions.Concat(ControllerConstants.HatariFireActions);
        return ControllerConstants.DirectionActions.Concat(ControllerConstants.SingleFireActions);
    }
