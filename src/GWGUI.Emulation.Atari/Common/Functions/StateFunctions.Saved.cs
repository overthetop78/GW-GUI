...
internal static class SavedStateFunctions
{
        ...
        IEmulatorCore core)
    {
        if (header.FormatVersion != StateConstants.CurrentFormatVersion)
            throw Invalid(ErrorCode.StateIncompatible, StateConstants.UnsupportedFormatError);
        if (header.Core != core.Emulator || !string.Equals(header.CoreName, core.CoreName, StringComparison.Ordinal)
            ...
            throw Invalid(ErrorCode.StateIncompatible, StateConstants.CoreMismatchError);
        if (header.Model != configuration.Model)
            throw Invalid(ErrorCode.StateIncompatible, StateConstants.ModelMismatchError);
        if (!string.Equals(header.ContentSha256, ContentHash(configuration), StringComparison.OrdinalIgnoreCase))
            throw Invalid(ErrorCode.StateIncompatible, StateConstants.ContentMismatchError);
        if (!IsCompatibleConfigurationHash(configuration, header.ConfigurationSha256))
            throw Invalid(ErrorCode.StateIncompatible, StateConstants.ConfigurationMismatchError);
    }

    internal static void ValidatePayloadSize(int length)
    {
        if (length <= StateConstants.EmptyLength)
            throw Invalid(ErrorCode.StateInvalid, StateConstants.EmptyPayloadError);
        if (length > CommonConstants.MaximumStateSize)
            throw Invalid(ErrorCode.StateInvalid, StateConstants.PayloadTooLargeError);
    }
    ...
    private static string HashPath(string path)
    {
        ...
        if (!Directory.Exists(path)) throw new FileNotFoundException(StateConstants.ContentPathMissingError, path);
                     ...
                     .Order(StringComparer.OrdinalIgnoreCase))
        {
            ...
            while ((read = stream.Read(buffer)) > StateConstants.EmptyLength)
                hash.AppendData(buffer.AsSpan(StateConstants.FirstBufferIndex, read));
        }
