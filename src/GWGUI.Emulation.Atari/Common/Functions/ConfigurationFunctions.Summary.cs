...
internal static class ConfigurationSummaryFunctions
{
    internal static EmulationConfigurationSummary Create(MachineConfiguration configuration)
    {
        ...
        if (configuration.Options.ContainsKey(ConfigurationOptionConstants.MainMemory)
            || configuration.Options.ContainsKey(SettingsConstants.AlternateMemory))
        {
            var main = LongOption(configuration.Options, ConfigurationOptionConstants.MainMemory);
        ...
        if (configuration.Firmwares.Count > 0)
            details.Add(string.Join(ConfigurationSummaryFunctionsConstants.Value, configuration.Firmwares.Select(Firmware)));
    ...
    {
        var role = firmware.Category == FirmwareCategory.Tos ? ConfigurationSummaryFunctionsConstants.TOS : firmware.Category.ToString();
