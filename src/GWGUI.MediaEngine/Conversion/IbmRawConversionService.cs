...
public sealed class IbmRawConversionService(IsoScpSectorImageReader scpReader, IbmRawImageReader reader, IbmRawImageWriter writer)
{
    ...
    public static bool CanCreate(string formatId, string extension) => !formatId.Equals(DiskImageFormatIds.IbmScan, StringComparison.OrdinalIgnoreCase) && Formats.Floppy.Raw.IbmPcGeometryCatalog.TryFromFormatId(formatId, out _) && (extension.Equals(DiskImageFileExtensions.Ima, StringComparison.OrdinalIgnoreCase) || extension.Equals(DiskImageFileExtensions.Img, StringComparison.OrdinalIgnoreCase));
