...
internal sealed class AtariRecognizedImageNormalizer : IRecognizedImageNormalizer
{
    ...
    public bool TryNormalize(SectorImage image, string readerId, FileSystemVolume volume, out SectorImage normalized)
    {
        ...
        if (image.FormatId.StartsWith(DiskImageFormatIds.IbmPrefix, StringComparison.OrdinalIgnoreCase) && AtariProgramDetector.ContainsProgram(volume.Entries))
        {
            normalized = image.WithFormatId(DiskImageFormatIds.AtariStFromCapacity(image.Capacity));
            return true;
        }
        return false;
    }
