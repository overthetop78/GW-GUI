...
public sealed record BbcDfsGeometry(string FormatId, int Cylinders, int Heads)
{
    ...
    public static BbcDfsGeometry? Find(int heads, int capacity) => new[] { Ssd40, Ssd80, Dsd40, Dsd80 }.SingleOrDefault(geometry => geometry.Heads == heads && geometry.Capacity == capacity);
}
