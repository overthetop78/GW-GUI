...
public sealed class ScpReader : IScpReader, IMediaImageReader
{
    private static readonly IReadOnlySet<string> SupportedFormatIds = new[] { DiskImageFormatIds.RawScp }.ToFrozenSet(StringComparer.OrdinalIgnoreCase);
    private static readonly IReadOnlySet<string> SupportedExtensions = new[] { DiskImageFileExtensions.Scp }.ToFrozenSet(StringComparer.OrdinalIgnoreCase);
    private static readonly IReadOnlyList<ReadOnlyMemory<byte>> SupportedSignatures = [ScpFormatConstants.FileSignature.ToArray()];
    ...
    async ValueTask<bool> IMediaImageReader.CanReadAsync(MediaRecognitionContext context, CancellationToken cancellationToken)
    {
        ...
        if (context.Length < ScpFormatConstants.HeaderLength) return false;
        var signature = await context.ReadHeaderAsync(ScpFormatConstants.SignatureLength, cancellationToken).ConfigureAwait(false);
        return signature.Span.SequenceEqual(ScpFormatConstants.FileSignature);
    }
    ...
    async Task<MediaImageDocument> IMediaImageReader.ReadAsync(MediaRecognitionContext context, CancellationToken cancellationToken)
    {
        ...
        var image = Read(data.Span);
        Remember(context.Source.PrimaryPath, image);
        var metadata = ScpMetadataFunctions.Create(image);
        return MediaImageDocumentFactory.CreateFloppyFlux(context.Source, DiskImageFormatIds.RawScp, ScpProtectedTrackImageAdapter.Create(image), metadata);
    }
