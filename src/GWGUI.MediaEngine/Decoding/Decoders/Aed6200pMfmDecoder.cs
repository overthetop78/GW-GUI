    ...
    public FluxDecodeResult Decode(FluxRevolution revolution)
    {
        ...
        for (var offset = 0; offset + Aed6200pMfmFormat.HeaderPattern.Count * BitPrimitives.BitsPerByte <= stream.Bits.Length; offset++)
        {
            ...
            if (complete)
            {
                ...
                if (dataOffset >= 0)
                {
                    pairedData.Add(dataOffset); var dataBlockBytes = Aed6200pMfmFormat.DataMarkByteCount + size + Aed6200pMfmFormat.CrcByteCount; var dataEnd = (long)dataOffset + dataBlockBytes * MfmEncoding.EncodedByteBitCount;
