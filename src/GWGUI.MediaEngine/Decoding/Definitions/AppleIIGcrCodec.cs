...
internal static class AppleIIGcrCodec
{
    ...
    public static (byte[] Data, bool Valid, int EndOffset)? TryDecodeSixAndTwo(IReadOnlyList<bool> bits, int offset)
    {
        ...
        for (var index = 0; index < values.Length; index++) if (!AppleIIGcrFormat.InverseSixAndTwoTable.TryGetValue(encoded[index], out values[index])) return null;
    ...
    public static (byte[] Data, bool Valid, int EndOffset)? TryDecodeFiveAndThree(IReadOnlyList<bool> bits, int offset)
    {
        ...
        for (var index = 0; index < values.Length; index++) if (!AppleIIGcrFormat.InverseFiveAndThreeTable.TryGetValue(encoded[index], out values[index])) return null;
    ...
    public static byte[] EncodeFiveAndThree(IReadOnlyList<byte> source)
    {
        ...
        {
            var zero = source[sourceOffset++]; var one = source[sourceOffset++]; var two = source[sourceOffset++]; var three = source[sourceOffset++]; var four = source[sourceOffset++];
            top[chunk] = (byte)(zero >> 3); top[chunk + chunkSize] = (byte)(one >> 3); top[chunk + chunkSize * 2] = (byte)(two >> 3); top[chunk + chunkSize * 3] = (byte)(three >> 3); top[chunk + chunkSize * 4] = (byte)(four >> 3);
