...
internal sealed record ZMachineStoryHeader(byte Version, ushort HighMemory, ushort InitialProgramCounter, ushort Dictionary, ushort Objects, ushort Globals, ushort StaticMemory, int Length, ushort Checksum)
{
    /// <summary>Tente de lire et valider l'en-tête d'une histoire.</summary>
    public static bool TryParse(ReadOnlySpan<byte> story, out ZMachineStoryHeader header)
    {
        header = null!;
        ...
        if (length is < AppleInformXzipLayout.MinimumHeaderLength or > AppleInformXzipLayout.MaximumStorySectorCount * AppleInformXzipLayout.SectorSize || length > story.Length) return false;
