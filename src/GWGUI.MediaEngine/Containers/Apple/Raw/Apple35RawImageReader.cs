...
internal static class Apple35RawImageReader
{
    ...
    private static readonly LinearSectorImageGeometry MacintoshGeometry = new(MacintoshMfmGeometry.SectorSize, MacintoshMfmGeometry.CylinderCount, MacintoshMfmGeometry.HeadCount, MacintoshMfmGeometry.SectorsPerTrack);
    ...
    public static AppleRawImageReadResult Read(ReadOnlyMemory<byte> data, string extension, AppleRawImageLayout layout)
    {
        ...
        if (layout == AppleRawImageLayoutCatalog.Macintosh1440K && AppleRawImageProbe.LooksLikeProDos(data.Span)) return new(LinearSectorImageBuilder.Create(data, DiskImageFormatIds.AppleIIProDos, MacintoshGeometry), AppleRawImageMatchKind.ValidatedStructure);
    ...
    {
        if (layout == AppleRawImageLayoutCatalog.Macintosh1440K) return new(LinearSectorImageBuilder.Create(data, DiskImageFormatIds.Mac1440, MacintoshGeometry), AppleRawImageMatchKind.ValidatedStructure);
