namespace GWGUI.MediaEngine.Encoding;

public sealed class MicropolisMfmTrackEncoder : TrackEncoderBase
{
    public override string Id => FluxCodecIds.MicropolisMfm;
    public override string DisplayName => "Micropolis MFM";

    protected override IReadOnlyList<bool> EncodeBits(TrackEncodeRequest request)
    {
        var bits = TrackEncoding.Bits();
        foreach (var sector in request.Sectors)
        {
            if (sector.Data.Count != 256) throw new ArgumentException("Micropolis sectors contain 256 bytes.");
            var record = new List<byte> { 0xff, (byte)request.Cylinder, (byte)sector.Number };
            record.AddRange(Enumerable.Repeat((byte)0, 10));
            record.AddRange(sector.Data);
            record.Add(Checksum(record.Skip(1)));
            record.AddRange(Enumerable.Repeat((byte)0, 5));
            bits.Mfm(new byte[40]);
            bits.Mfm(record);
            bits.Gap(128);
        }
        return bits;
    }

    private static byte Checksum(IEnumerable<byte> data)
    {
        var value = 0;
        foreach (var item in data) { if (value > 255) value -= 255; value += item; }
        return (byte)value;
    }
}
