...
namespace GWGUI.MediaEngine.Images.Formats.Floppy.I86f;

/// <summary>Lit un conteneur 86F et restitue ses pistes sous forme de cellules de bits normalisées.</summary>
public sealed class I86fReader : IMediaImageReader
{
    private static readonly IReadOnlySet<string> SupportedFormatIds = new[] { DiskImageFormatIds.RawI86f }.ToFrozenSet(StringComparer.OrdinalIgnoreCase);
    private static readonly IReadOnlySet<string> SupportedExtensions = new[] { DiskImageFileExtensions.I86f }.ToFrozenSet(StringComparer.OrdinalIgnoreCase);
    private static readonly IReadOnlyList<ReadOnlyMemory<byte>> SupportedSignatures = [CreateSignature()];
    ...
    async ValueTask<bool> IMediaImageReader.CanReadAsync(MediaRecognitionContext context, CancellationToken cancellationToken)
    {
        ...
        if (context.Length < I86fLayout.MinimumFileLength) return false;
        var signature = await context.ReadHeaderAsync(I86fFormat.SignatureLength, cancellationToken).ConfigureAwait(false);
        return BinaryPrimitives.ReadUInt32LittleEndian(signature.Span) == I86fFormat.Signature;
    }
    ...
    async Task<MediaImageDocument> IMediaImageReader.ReadAsync(MediaRecognitionContext context, CancellationToken cancellationToken)
    {
        ...
        var image = await ReadAsync(data, cancellationToken).ConfigureAwait(false);
        var metadata = new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ["86f.flags"] = ((ushort)image.Flags).ToString(CultureInfo.InvariantCulture)
        };
        return MediaImageDocumentFactory.CreateFloppyFlux(context.Source, DiskImageFormatIds.RawI86f, I86fProtectedTrackImageAdapter.Create(image), metadata);
    }
    ...
    {
        var signature = new byte[I86fFormat.SignatureLength];
        BinaryPrimitives.WriteUInt32LittleEndian(signature, I86fFormat.Signature);
        return signature;
    }
}
