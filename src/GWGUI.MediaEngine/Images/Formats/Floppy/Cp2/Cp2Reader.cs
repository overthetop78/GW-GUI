...
namespace GWGUI.MediaEngine.Images.Formats.Floppy.Cp2;

/// <summary>Lit les secteurs capturés par SNATCH-IT pour Copy II PC.</summary>
public sealed class Cp2Reader : IMediaImageReader
{
    private static readonly IReadOnlySet<string> SupportedFormatIds = new[]
    {
        DiskImageFormatIds.Ibm160,
        DiskImageFormatIds.Ibm180,
        DiskImageFormatIds.Ibm320,
        DiskImageFormatIds.Ibm360,
        DiskImageFormatIds.Ibm720,
        DiskImageFormatIds.Ibm800,
        DiskImageFormatIds.Ibm1200,
        DiskImageFormatIds.Ibm1440,
        DiskImageFormatIds.Ibm1680,
        DiskImageFormatIds.IbmDmf,
        DiskImageFormatIds.Ibm2880
    }.ToFrozenSet(StringComparer.OrdinalIgnoreCase);
    private static readonly IReadOnlySet<string> SupportedExtensions = new[] { DiskImageFileExtensions.Cp2 }.ToFrozenSet(StringComparer.OrdinalIgnoreCase);
    private static readonly IReadOnlyList<ReadOnlyMemory<byte>> SupportedSignatures = [Cp2Format.Signature.ToArray()];
    ...
    async ValueTask<bool> IMediaImageReader.CanReadAsync(MediaRecognitionContext context, CancellationToken cancellationToken)
    {
        ...
        if (context.Length < Cp2Layout.MinimumFileLength) return false;
        var signature = await context.ReadHeaderAsync(Cp2Format.SignatureLength, cancellationToken).ConfigureAwait(false);
        return signature.Span.SequenceEqual(Cp2Format.Signature);
    }
    ...
    private static SectorImage BuildImage(IReadOnlyDictionary<SectorAddress, byte[]> sectors, CancellationToken cancellationToken)
    {
        ...
        if (heads is <= 0 or > Constants.DiskGeometryConstants.DoubleSidedHeadCount || sectorsPerTrack <= 0)
