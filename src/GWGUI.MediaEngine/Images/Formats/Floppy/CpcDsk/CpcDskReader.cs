...
public sealed class CpcDskReader : IMediaImageReader
{
    private static readonly IReadOnlySet<string> SupportedFormatIds = new[] { CpcDskFormat.FormatId }.ToFrozenSet(StringComparer.OrdinalIgnoreCase);
    private static readonly IReadOnlySet<string> SupportedExtensions = new[] { DiskImageFileExtensions.Dsk, DiskImageFileExtensions.Edsk }.ToFrozenSet(StringComparer.OrdinalIgnoreCase);
    private static readonly IReadOnlyList<ReadOnlyMemory<byte>> SupportedSignatures = [CpcDskFormat.StandardSignatureBytes.ToArray(), CpcDskFormat.ExtendedSignatureBytes.ToArray()];
    ...
    async ValueTask<bool> IMediaImageReader.CanReadAsync(MediaRecognitionContext context, CancellationToken cancellationToken)
    {
        ...
        if (context.Length < CpcDskLayout.DiskInformationBlockSize) return false;
        var header = await context.ReadHeaderAsync(CpcDskLayout.DiskSignatureLength, cancellationToken).ConfigureAwait(false);
        return header.Span.StartsWith(CpcDskFormat.StandardSignatureBytes) || header.Span.StartsWith(CpcDskFormat.ExtendedSignatureBytes);
    }
