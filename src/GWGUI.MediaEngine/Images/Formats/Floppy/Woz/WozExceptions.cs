namespace GWGUI.MediaEngine.Images.Formats.Floppy.Woz;
...
internal static class WozExceptions
{
    ...
    public static NotSupportedException UnsupportedDiskType(byte observedDiskType) => new($"WOZ disk type {observedDiskType} is not supported; an Apple II 5.25-inch disk is required.");
    ...
    public static InvalidDataException MissingRequiredChunk(string chunkId) => new($"The required WOZ chunk '{chunkId}' is missing or incomplete.");
    ...
    /// <returns>Exception contenant l’identifiant du chunk.</returns>
    public static InvalidDataException TruncatedChunk(string chunkId) => new($"The WOZ {chunkId} chunk is truncated.");
    ...
    public static InvalidDataException TrackReferenceOutOfBounds(int track, int descriptor) => new($"WOZ track {track} references out-of-bounds descriptor {descriptor}.");
