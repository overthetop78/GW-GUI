...
internal static class ArburgChecksum
{
    ...
    public static bool IsValid(IReadOnlyList<byte> block, int usefulSize)
    {
        ...
        return block[usefulSize] == (byte)checksum && block[usefulSize + 1] == (byte)(checksum >> GWGUI.MediaEngine.Functions.BitPrimitives.BitsPerByte);
    }
    ...
    public static byte[] CreateBlock(IReadOnlyList<byte> data, int totalSize)
    {
        ...
        block[data.Count + 1] = (byte)(checksum >> GWGUI.MediaEngine.Functions.BitPrimitives.BitsPerByte);
        return block;
    }
}
