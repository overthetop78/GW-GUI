...
public sealed class AmigaMfmTrackEncoder : TrackEncoderBase
{
    ...
    protected override IReadOnlyList<bool> EncodeBits(TrackEncodeRequest request)
    {
        ...
        for (var sectorIndex = 0; sectorIndex < request.Sectors.Count; sectorIndex++)
        {
            ...
            bits.Raw((byte)(AmigaMfmFormat.SyncWord >> GWGUI.MediaEngine.Functions.BitPrimitives.BitsPerByte), unchecked((byte)AmigaMfmFormat.SyncWord), (byte)(AmigaMfmFormat.SyncWord >> GWGUI.MediaEngine.Functions.BitPrimitives.BitsPerByte), unchecked((byte)AmigaMfmFormat.SyncWord));
            bits.Mfm(encoded);
