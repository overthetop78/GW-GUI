namespace GWGUI.Emulation.Amiga.Common.Constants;

internal static class ConfigurationStoreConstants
{
    internal const string MachineJson = "machine.json";
    internal const string Json = "*.json";
    internal const string N = "N";
    internal const string Tmp = ".tmp";
    internal const int ReplacementRetryCount = 5;
    internal const int ReplacementRetryDelayMilliseconds = 50;
    internal const string Value = "..";
}

internal static class ConfigurationSummaryFunctionsConstants
{
    internal const string OptionCpuModel = "puae_cpu_model";
    internal const string OptionVideoStandard = "puae_video_standard";
    internal const string NTSC = "NTSC";
    internal const string NTSC2 = "/NTSC";
    internal const string PAL = "/PAL";
    internal const string OptionChipmemSize = "puae_chipmem_size";
    internal const string OptionBogomemSize = "puae_bogomem_size";
    internal const string OptionFastmemSize = "puae_fastmem_size";
    internal const string OptionZ3memSize = "puae_z3mem_size";
    internal const string Value0 = "0";
    internal const string GwguiFloppyDriveCount = "gwgui_floppy_drive_count";
    internal const string GwguiHardDriveCount = "gwgui_hard_drive_count";
    internal const string GwguiCdDriveEnabled = "gwgui_cd_drive_enabled";
    internal const string Enabled = "enabled";
    internal const string Disabled = "disabled";
    internal const string CD = "CD";
    internal const string Value = " / ";
    internal const string AudioOn = "Audio On";
    internal const string AudioOff = "Audio Off";
    internal const string Value2 = "…";
    internal const string Value22 = "2";
    internal const string Value4 = "4";
    internal const string Value6 = "6";
    internal const string Value7 = "7";
    internal const string D3D11 = "D3D11";
    internal const string D11232045301DD = @"(?<!\d)(?:1\.[123]|2\.0[45]|3\.[01])(?:\.\d+)?(?!\d)";
}

internal static class ConfigurationValidationFunctionsConstants
{
    internal const string AMIROMTYPE1 = "AMIROMTYPE1";
}
