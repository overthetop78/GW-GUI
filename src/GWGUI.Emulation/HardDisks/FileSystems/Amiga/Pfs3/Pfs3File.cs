...
namespace Hst.Amiga.FileSystems.Pfs3
{
    ...
    public static class Pfs3File
    {
	    ...
	    /// <returns></returns>
	    public static async Task<IPfs3Entry> Open(Pfs3ObjectInfo parentfi, string fullname, Amiga.FileSystems.FileMode mode,
		    bool overwrite, Pfs3GlobalData g)
        {
	        ...
	        var pathfi = new Pfs3ObjectInfo();
            var filefi = new Pfs3ObjectInfo();
			...
			/* Get path to file */
			var filename = await Pfs3Directory.GetFullPath(parentfi.Clone(), fullname, pathfi, g);
			if (filename == null)
			{
			...
			/* try to locate file */
			var found = await Pfs3Directory.FindObject(pathfi, filename, filefi, g);
            ...
            if (found)
            {
	            ...
	            isLink = filefi.file.direntry.type == Pfs3Constants.ST_LINKFILE ||
	                     filefi.file.direntry.type == Pfs3Constants.ST_LINKDIR;
	            originalSize = filefi.file.direntry.Size;

				/* softlinks cannot directly be opened */
				if (Pfs3Macro.IsSoftLink(filefi))
				{
				...
				// #if DELDIR
				if ((Pfs3Macro.IsVolume(filefi) || Pfs3Macro.IsDelDir(filefi) || Pfs3Macro.IsDir(filefi)))
				// #else
			...
			// #if MU_CHECKDIR
			// 	if (IsVolume(pathfi))
			// 		memset(&path_extrafields, 0, sizeof(struct Pfs3ExtraFields));
			// 	else
			// #if DELDIR
			...
			switch(mode)
			{
				...
				case Amiga.FileSystems.FileMode.Read:
					...
					var hasReadProtectionBit =
						(filefi.file.direntry.protection & Pfs3Constants.FIBF_READ) == 0;
					...
					type.flags.access = Pfs3Constants.ET_SHAREDREAD;
					break;
				...
				case Amiga.FileSystems.FileMode.Write:
					if (overwrite || !found)
					{
						await Pfs3Directory.NewFile(found, parentfi, filename, filefi, overwrite, g);
					}
					type.flags.access = Pfs3Constants.ET_EXCLWRITE;
					break;
			...
			// 	return DOSFALSE;
			IPfs3Entry fileFe;
			if ((fileFe = await Pfs3Lock.MakeListEntry(filefi, type, g)) == null)
			{
			...
			// 		return DOSFALSE;
			// 	}
			if (!Pfs3Lock.AddListEntry(fileFe.ListEntry, g))
			{
				//DB(Trace(1, "dd_Open", "AddListEntry failed"));
			...
			if (fileFe is Pfs3FileEntry fileentry)
			{
        ...
        public static async Task Close(Pfs3FileEntry fe, Pfs3GlobalData g)
        {
#if DEBUG
            ...
            if (fe.checknotify)
            {
                Pfs3Cache.ClearSearchInDirCache(fe.le.dirblocknr, g);

                Pfs3VolumeOperations.CheckVolume(fe.le.volume, true, g);
                await Pfs3Lru.UpdateLE(fe.le, g);
                await Pfs3Directory.Touch(fe.le.info, g);
                var size = Pfs3Directory.GetDEFileSize(fe.le.info.file.direntry, g);
                if (fe.originalsize != size)
                {
                    await Pfs3Directory.UpdateLinks(fe.le.info.file.direntry, g);
                }
            ...
            Pfs3Lock.RemoveListEntry(fe, g);
        }

        public static void MakeSharedFileEntriesAndClear(Pfs3GlobalData g)
        {
            foreach (var fileEntry in g.currentvolume.fileentries)
            {
                fileEntry.ListEntry.type.flags.access = Pfs3Constants.ET_SHAREDWRITE;
            }
