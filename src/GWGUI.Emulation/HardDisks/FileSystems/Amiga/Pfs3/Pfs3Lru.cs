...
/* Allocate LRU queue
*/
        public static void InitLRU(Pfs3GlobalData g, ushort reservedBlksize)
        {
            ...
            g.glob_lrudata.LRUqueue = new LinkedList<Pfs3LruCachedBlock>();
            g.glob_lrudata.LRUpool = new LinkedList<Pfs3LruCachedBlock>();
            ...
            g.glob_lrudata.LRUarray = g.glob_lrudata.useLruArray ? Enumerable.Range(1, (int)g.glob_lrudata.poolsize)
                    .Select(_ => new Pfs3LruCachedBlock(new Pfs3CachedBlock())).ToArray() : Array.Empty<Pfs3LruCachedBlock>();
        }
        ...
        /// <param name="g"></param>
        /// <returns></returns>
        public static Pfs3CachedBlock CheckCache(LinkedList<Pfs3CachedBlock>[] list, ushort mask, uint blocknr, Pfs3GlobalData g)
        {
            for (var block = Pfs3Macro.HeadOf(list[(blocknr / 2) & mask]);
                 block != null;
                 block = block.Next)
            {
        ...
        /// <param name="g"></param>
        /// <returns></returns>
        public static Pfs3CachedBlock CheckCache(IDictionary<uint, Pfs3CachedBlock> list, uint blocknr, Pfs3GlobalData g)
        {
        ...
        public static void MakeLRU(Pfs3CachedBlock blk, Pfs3GlobalData g)
        {
#if DEBUG
            ...
            Pfs3Macro.MinRemoveLru(blk, g);

            // MinAddHead(g.glob_lrudata.LRUqueue, LRU_CHAIN(blk));
            Pfs3Macro.MinAddHead(g.glob_lrudata.LRUqueue, new Pfs3LruCachedBlock(blk));
        }

        public static void FreeLRU(Pfs3CachedBlock blk, Pfs3GlobalData g)
        {
#if DEBUG
            ...
            Pfs3Macro.MinAddHead(g.glob_lrudata.LRUpool, new Pfs3LruCachedBlock(blk));
        }
...
*/
        public static async Task<Pfs3CachedBlock> AllocLRU(Pfs3GlobalData g)
        {
            ...
            */
            LinkedListNode<Pfs3LruCachedBlock> lrunode;
            retry:
            if (g.glob_lrudata.LRUpool.Count == 0)
            {
                ...
                {
                    /* skip locked blocks */
                    if (Pfs3Cache.ISLOCKED(lrunode.Value.cblk, g))
                        continue;
                    ...
                    if (lrunode.Value.cblk.changeflag)
                    {
                        ...
                        Pfs3Update.UpdateDatestamp(lrunode.Value.cblk, g);
                        if (!(await Pfs3Disk.RawWrite(g.stream, lrunode.Value.cblk.blk, g.currentvolume.rescluster,
            ...
            {
                lrunode = Pfs3Macro.HeadOf(g.glob_lrudata.LRUpool);
                goto ready;
            }
            ...
            if (!g.glob_lrudata.useLruArray)
            {
                for (j = 0; j < NEW_LRU_ENTRIES; j++)
                {
                    ...
                    Pfs3Macro.MinAddHead(g.glob_lrudata.LRUpool, new Pfs3LruCachedBlock(new Pfs3CachedBlock()));
                }
            ...
            var nlru = new Pfs3LruCachedBlock[g.glob_lrudata.poolsize + NEW_LRU_ENTRIES];
            for (j = 0; j < NEW_LRU_ENTRIES; j++)
            {
                if (nlru == null)
                    break;
                ...
                nlru[j + g.glob_lrudata.poolsize] = new Pfs3LruCachedBlock(new Pfs3CachedBlock());
            ...
            {
                //MinAddHead(&g->glob_lrudata.LRUpool, g.glob_lrudata.LRUarray[g.glob_lrudata.poolsize]);
                Pfs3Macro.MinAddHead(g.glob_lrudata.LRUpool,
            ...
            Pfs3Macro.MinRemoveLru(lrunode.Value.cblk, g);
            Pfs3Macro.MinAddHead(g.glob_lrudata.LRUqueue, lrunode.Value);
            ...
            //  LOCK(&lrunode->cblk);
            Pfs3Cache.LOCK(lrunode.Value.cblk, g);
            return lrunode.Value.cblk;
        }

/* Adds a block to the ReservedToBeFreedCache
 */
        public static void ResToBeFreed(uint blocknr, Pfs3GlobalData g)
        {
            ...
            if (blocknr != 0)
            {
                ...
                {
                    /* reallocate cache */
                    uint newsize = alloc_data.rtbf_size != 0 ? alloc_data.rtbf_size * 2 : Pfs3Constants.RTBF_CACHE_SIZE;
...
*/
        public static void FlushBlock(Pfs3CachedBlock block, Pfs3GlobalData g)
        {
            // lockentry_t *le;
            //
            ...
            // MinRemove(block);
            Pfs3Macro.MinRemove(block, g);

            /* decouple references */
            if (Pfs3Macro.IsDirBlock(block))
            {
...
// // #if DELDIR
// // le->nextentry.direntry = (struct Pfs3DirEntry *)SPECIAL_FLUSHED;
...
//                 }
        ...
        private static void ClearBlock(Pfs3CachedBlock cachedBlock, Pfs3GlobalData g)
        {
#if DEBUG
...
*/
        public static void UpdateReference(uint blocknr, Pfs3CachedBlock blk, Pfs3GlobalData g)
        {
            Pfs3LockEntry le;
 ...
 */
        public static async Task UpdateLE(Pfs3ListEntry le, Pfs3GlobalData g)
        {
...
// #if DELDIR
	        if (le == null || le.info.deldir.special <= Pfs3Constants.SPECIAL_DELFILE)
// #else
...
// #endif
            ...
            if (le.dirblocknr != 0)
                await Pfs3Directory.LoadDirBlock (le.dirblocknr, g);

            MakeLRU (le.info.file.dirblock, g);
            Pfs3Macro.Lock(le.info.file.dirblock, g);
        }
    }
}
