namespace Hst.Amiga.FileSystems.Pfs3
{
    ...
    public static class Pfs3Formatter
    {
        ...
        public static async Task FormatPartition(Stream stream, PartitionBlock partitionBlock, string diskName)
        {
            ...
            var g = Pfs3Init.CreateGlobalData(partitionBlock.Sectors, partitionBlock.BlocksPerTrack,
            ...
            if (g.TotalSectors > Pfs3Constants.MAXDISKSIZE)
            {
            ...
            g.currentvolume = volume = await Pfs3VolumeOperations.MakeVolumeData(rootBlock, g);
            ...
            rootBlock.Options |= Pfs3RootBlock.Pfs3DiskOptions.MODE_EXTENSION;

            await Pfs3Init.InitModules(g.currentvolume, true, g);
            ...
            await Pfs3Directory.SetDeldir(2, g);
            rootBlock.Options |= Pfs3RootBlock.Pfs3DiskOptions.MODE_DELDIR | Pfs3RootBlock.Pfs3DiskOptions.MODE_SUPERDELDIR;
            g.dirty = true;

            //
            await Pfs3Update.UpdateDisk(g);
            Pfs3VolumeOperations.FreeVolumeResources(volume, g);
 ...
 */
        public static async Task MakeBootBlock(Pfs3GlobalData g)
        {
            // struct bootblock *bbl;
            // ULONG error;

            var bbl = new Pfs3BootBlock
            {
...
// #endif
            ...
            var buffer = await Pfs3BootBlockWriter.MakeBootBlock(bbl, (int)g.blocksize);
            if (!await Pfs3Disk.RawWrite(g.stream, buffer, 2, Pfs3Constants.BOOTBLOCK1, g))
            {
        ...
        public static Pfs3RootBlock MakeRootBlock(string diskName, Pfs3GlobalData g)
        {
            var rootBlock = new Pfs3RootBlock
            {
                DiskType = Pfs3Constants.ID_PFS_DISK,
                Datestamp = 1,
                ...
                Options = Pfs3RootBlock.Pfs3DiskOptions.MODE_HARDDISK | Pfs3RootBlock.Pfs3DiskOptions.MODE_SPLITTED_ANODES |
                          Pfs3RootBlock.Pfs3DiskOptions.MODE_DIR_EXTENSION |
                          Pfs3RootBlock.Pfs3DiskOptions.MODE_SIZEFIELD | Pfs3RootBlock.Pfs3DiskOptions.MODE_DATESTAMP |
                          Pfs3RootBlock.Pfs3DiskOptions.MODE_EXTROVING |
                          Pfs3RootBlock.Pfs3DiskOptions.MODE_LONGFN
            };

            // determine reserved blocksize
            uint resblocksize = 1024;
            if (g.TotalSectors > Pfs3Constants.MAXSMALLDISK)
            {
                rootBlock.Options |= Pfs3RootBlock.Pfs3DiskOptions.MODE_SUPERINDEX;
                g.SuperMode = true;
                if (g.TotalSectors > Pfs3Constants.MAXDISKSIZE1K)
                {
                    resblocksize = 2048;
                    if (g.TotalSectors > Pfs3Constants.MAXDISKSIZE2K)
                    {
                        resblocksize = 4096;
                    }

                    rootBlock.DiskType = Pfs3Constants.ID_PFS2_DISK;
                    //NormalErrorMsg(AFS_WARNING_EXPERIMENTAL_DISK, NULL, 1);
                }
            }

            Pfs3Init.Initialize(g);
            rootBlock.Options |= Pfs3RootBlock.Pfs3DiskOptions.MODE_HARDDISK;

            Pfs3Lru.InitLRU(g, (ushort)resblocksize);
        ...
        public static async Task MakeBitmap(Pfs3GlobalData g)
        {
            ...
            {
                await Pfs3Allocation.NewBitmapBlock(i, g);
            }
        }

        public static async Task MakeRootDir(Pfs3GlobalData g)
        {
            Pfs3CachedBlock blk;
            uint blocknr, anodenr;

                blocknr = Pfs3Allocation.AllocReservedBlock(g);
                anodenr = await Pfs3Anodes.AllocAnode(0, g);
            blk = await Pfs3Directory.MakeDirBlock(blocknr, anodenr, anodenr, 0, g);
        }

/* Create rootblockextension. Needed for AFS 2.3 options.
 */
        public static bool MakeRBlkExtension (Pfs3GlobalData g)
        {
            Pfs3VolumeData volume = g.currentvolume;
            uint blocknr;
            ...
            g.RootBlock.Options |= Pfs3RootBlock.Pfs3DiskOptions.MODE_EXTENSION;
            volume.rootblockchangeflag = true;
            return true;
        }

        public static Pfs3CachedBlock MakeFormatRBlkExtension(Pfs3RootBlock rootBlock, Pfs3GlobalData g)
        {
            // if (!(rext = AllocBufmem (sizeof(struct Pfs3DoctorCachedBlock) + rbl->reserved_blksize, g)))
            // return FALSE;
            //memset (rext, 0, sizeof(struct Pfs3DoctorCachedBlock) + rbl->reserved_blksize);
            ...
            var rext = new Pfs3CachedBlock()
            {
                ...
                changeflag = true,
                blk = new Pfs3RootBlockExtension
                {
                    id = Pfs3Constants.EXTENSIONID,
                    pfs2version = (Pfs3Constants.VERNUM << 16) + Pfs3Constants.REVNUM,
        ...
        public static uint CalcNumReserved(Pfs3GlobalData g, uint resblocksize)
        {
            uint taken, i;
            ...
            taken += 10;
            taken = Math.Min(Pfs3Constants.MAXNUMRESERVED, taken);
        ...
        /// <returns></returns>
        public static int CalculateRootBlockAndReservedBitmapBlockCount(Pfs3RootBlock rbl, uint numReserved)
        {
        ...
        public static void MakeReservedBitmap(Pfs3RootBlock rbl, uint numReserved, Pfs3GlobalData g)
        {
            ...
            var bmb = new Pfs3BitmapBlock(reservedLongsPerBmb);

            /* init bitmapblock header */
            bmb.id = Pfs3Constants.BMBLKID;
            bmb.seqnr = 0;
