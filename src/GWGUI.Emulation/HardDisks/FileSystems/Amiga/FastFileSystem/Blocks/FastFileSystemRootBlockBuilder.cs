namespace Hst.Amiga.FileSystems.FastFileSystem.Blocks
{
    ...
    public static class FastFileSystemRootBlockBuilder
    {
        public static byte[] Build(FastFileSystemRootBlock rootBlock, int blockSize)
        {
            if (rootBlock.SecType != FastFileSystemConstants.ST_ROOT)
            {
            ...
            for (var i = 0; i < FastFileSystemConstants.BM_SIZE; i++)
            {
            ...
            blockBytes.WriteStringWithLength(blockBytes.Length - 0x50, rootBlock.DiskName, FastFileSystemConstants.MAXNAMELEN);
