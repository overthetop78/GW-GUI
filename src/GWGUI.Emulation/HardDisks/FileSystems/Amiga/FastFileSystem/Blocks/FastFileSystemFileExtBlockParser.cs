namespace Hst.Amiga.FileSystems.FastFileSystem.Blocks
{
    ...
    public static class FastFileSystemFileExtBlockParser
    {
        public static FastFileSystemFileExtBlock Parse(byte[] blockBytes)
        {
            var type = BigEndianConverter.ConvertBytesToInt32(blockBytes);

            if (type != FastFileSystemConstants.T_LIST)
            {
            ...
            return new FastFileSystemFileExtBlock(blockBytes.Length)
            {
