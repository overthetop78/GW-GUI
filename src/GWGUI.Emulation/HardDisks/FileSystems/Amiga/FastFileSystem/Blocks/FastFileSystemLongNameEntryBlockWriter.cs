...
namespace Hst.Amiga.FileSystems.FastFileSystem.Blocks
{
    ...
    /// </summary>
    public static class FastFileSystemLongNameEntryBlockWriter
    {
        public static byte[] Build(FastFileSystemEntryBlock entryBlock, int blockSize)
        {
            ...
            blockBytes.WriteStringWithLength(blockBytes.Length - 0xb8, entryBlock.Name, FastFileSystemConstants.LNFSMAXNAMELEN);

            if (!string.IsNullOrEmpty(entryBlock.Comment))
            {
                var nameAndCommendSpaceLeft = FastFileSystemConstants.LNFSNAMECMMTLEN - entryBlock.Name.Length + 1;
                ...
                blockBytes.WriteStringWithLength(blockBytes.Length - 0xb8 + entryBlock.Name.Length + 1, entryBlock.Comment, FastFileSystemConstants.LNFSMAXNAMELEN);
            }
