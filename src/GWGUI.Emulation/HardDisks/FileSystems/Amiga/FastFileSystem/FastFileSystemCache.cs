namespace Hst.Amiga.FileSystems.FastFileSystem
{
    ...
    public static class FastFileSystemCache
    {
        public static async Task<IEnumerable<FastFileSystemEntry>> ReadEntries(FastFileSystemVolumeState vol, uint dir, bool recursive = false)
        {
            var list = new List<FastFileSystemEntry>();

            var parent = await FastFileSystemDisk.ReadEntryBlock(vol, dir);
            ...
            do
            {
                ...
                int offset;
                var n = offset = 0;
                var dirC = await FastFileSystemDisk.ReadDirCacheBlock(vol, nSect);
        ...
        public static async Task AddInCache(FastFileSystemVolumeState vol, FastFileSystemEntryBlock parent, FastFileSystemEntryBlock entry)
        {
            FastFileSystemDirCacheBlock dirCacheBlock;
            ...
            do
            {
                dirCacheBlock = await FastFileSystemDisk.ReadDirCacheBlock(vol, nSect);
                offset = 0;
                var n = 0;
            ...
            else
            {
                ...
                var nCache = FastFileSystemBitmap.AdfGet1FreeBlock(vol);
                if (nCache == uint.MaxValue) // -1
                {
                    throw new IOException("nCache==-1");
                }

                if (!(parent.SecType == FastFileSystemConstants.ST_ROOT || parent.SecType == FastFileSystemConstants.ST_DIR))
                {
                    throw new IOException("Invalid sec type for parent block");
                }

                /* create a new dir cache block */
                var newDirCacheBlock = new FastFileSystemDirCacheBlock(vol.FileSystemBlockSize)
                {
                ...
                await FastFileSystemDisk.WriteDirCacheBlock(vol, nCache, newDirCacheBlock);
                dirCacheBlock.NextDirC = nCache;
            }

            await FastFileSystemDisk.WriteDirCacheBlock(vol, dirCacheBlock.HeaderKey, dirCacheBlock);
        }

        public static FastFileSystemCacheEntry ConvertEntryBlockToCacheEntry(FastFileSystemEntryBlock entry)
        {
            return new FastFileSystemCacheEntry
            {
                Header = entry.HeaderKey,
                Size = entry.SecType == FastFileSystemConstants.ST_FILE ? entry.ByteSize : 0,
        ...
        public static async Task UpdateCache(FastFileSystemVolumeState vol, FastFileSystemEntryBlock parent, FastFileSystemEntryBlock entry, bool entryLenChg)
        {
            var newCacheEntry = ConvertEntryBlockToCacheEntry(entry);
            ...
            do
            {
                var dirCacheBlock = await FastFileSystemDisk.ReadDirCacheBlock(vol, nSect);
                var offset = 0;
                var n = 0;
        ...
        public static async Task CreateEmptyCache(FastFileSystemVolumeState vol, FastFileSystemEntryBlock parent, uint nSect)
        {
            if (!(parent.SecType == FastFileSystemConstants.ST_ROOT || parent.SecType == FastFileSystemConstants.ST_DIR))
            {
            ...
            {
                nCache = FastFileSystemBitmap.AdfGet1FreeBlock(vol);
            ...
            var dirCacheBlock = new FastFileSystemDirCacheBlock(vol.FileSystemBlockSize)
            {
                Parent = parent.SecType == FastFileSystemConstants.ST_ROOT ? vol.RootBlockOffset : parent.HeaderKey,
                RecordsNb = 0,
                NextDirC = 0
            ...
            await FastFileSystemDisk.WriteDirCacheBlock(vol, nCache, dirCacheBlock);
        }
        ...
        /// <returns></returns>
        public static FastFileSystemCacheEntry GetCacheEntry(FastFileSystemDirCacheBlock dirCacheBlock, ref int ptr)
        {
            var cacheEntry = new FastFileSystemCacheEntry
            {
        ...
        public static int PutCacheEntry(FastFileSystemDirCacheBlock dirCacheBlock, ref int ptr, FastFileSystemCacheEntry cacheEntry)
        {
        ...
        public static async Task DeleteFromCache(FastFileSystemVolumeState vol, FastFileSystemEntryBlock parent, uint headerKey)
        {
                ...
                while (n < dirCacheBlock.RecordsNb && !found)
                {
                    ...
                    if (found)
                    {
                        ...
                        else
                        {
                            ...
                            */
                            FastFileSystemBitmap.AdfSetBlockFree(vol, dirCacheBlock.HeaderKey);
                            dirCacheBlock = await FastFileSystemDisk.ReadDirCacheBlock(vol, prevSect);
                            dirCacheBlock.NextDirC = 0;
                            await FastFileSystemDisk.WriteDirCacheBlock(vol, prevSect, dirCacheBlock);
