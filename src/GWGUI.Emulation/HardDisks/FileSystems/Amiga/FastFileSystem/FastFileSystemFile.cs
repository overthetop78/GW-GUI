namespace Hst.Amiga.FileSystems.FastFileSystem
{
    ...
    public static class FastFileSystemFile
    {
        public static async Task<Stream> Open(FastFileSystemVolumeState volume, FastFileSystemEntry entry, FileMode mode = FileMode.Read)
        {
            return await Open(volume, entry.Parent, entry.Name, mode);
        }

        public static async Task<Stream> Open(FastFileSystemVolumeState volume, uint parentSector, string name, FileMode mode,
            bool overwrite = false, bool ignoreProtectionBits = false)
        {
            ...
            var result = await FastFileSystemDirectory.GetEntryBlock(volume, parent.HashTable, name, false);
            ...
            if (entry != null && entry.SecType == FastFileSystemConstants.ST_LFILE)
            {
                nSect = entry.RealEntry;
                entry = await FastFileSystemDisk.ReadEntryBlock(volume, entry.RealEntry);
            }
            ...
            if (overwrite && entry != null && entry.SecType != FastFileSystemConstants.ST_FILE && entry.SecType != FastFileSystemConstants.ST_LFILE)
            {
            ...
            switch (mode)
            {
                case FileMode.Read when FastFileSystemMacro.hasR(entry.Access):
            ...
            var fileHdr = mode == FileMode.Write
                ? new FastFileSystemEntryBlock(volume.FileSystemBlockSize) {  SecType = FastFileSystemConstants.ST_FILE }
                : entry;
            ...
            {
            fileHdr = overwrite && nSect != uint.MaxValue ? entry : await FastFileSystemDirectory.CreateFile(volume, parent, name);
                if (overwrite)
                {
            ...
            var entryStream = new FastFileSystemEntryStream(volume, write, eof, fileHdr);
 ...
 *
 */
        public static async Task AdfFreeFileBlocks(FastFileSystemVolumeState vol, FastFileSystemEntryBlock entry)
        {
            // int i;
            ...
            {
                FastFileSystemBitmap.AdfSetBlockFree(vol, fileBlocks.data[i]);
            }
            for(var i=0; i < fileBlocks.nbExtens; i++)
            {
                FastFileSystemBitmap.AdfSetBlockFree(vol, fileBlocks.extens[i]);
            }

            if (entry.Extension != 0)
            {
                FastFileSystemBitmap.AdfSetBlockFree(vol, entry.Extension);
            }
 ...
 *
 */
        public static async Task<FastFileSystemFileBlocks> AdfGetFileBlocks(FastFileSystemVolumeState vol, FastFileSystemEntryBlock entry)
        {
            // int32_t n, m;
            // SECTNUM nSect;
            // struct bFileExtBlock extBlock;
            // int32_t i;
            var fileBlocks = new FastFileSystemFileBlocks
            {
            ...
            while(nSect!=0)
            {
                ...
                var extBlock = await FastFileSystemDisk.ReadFileExtBlock(vol, nSect);
 ...
 *
 */
        public static uint AdfFileRealSize(FastFileSystemVolumeState volume, uint size, uint blockSize, FastFileSystemFileBlocks fileBlocks) // int32_t *dataN, int32_t *extN
        {
            // int32_t data, ext;
