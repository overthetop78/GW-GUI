...
internal sealed class TagOptionsController
{
    ...
    public void RefreshLocalizedContent()
    {
        RefreshPreview();
        RefreshPresets();
        _section.TagVariables.ItemsSource = OptionsDefinitions.TagVariables
    ...
    private async void PatternEditingFinished(object sender, KeyboardFocusChangedEventArgs e)
    {
        if (!IsPreset(_section.TagPattern.Text) &&
            TagPatternFormatter.Remember(_settings.Conversion.RecentCustomTagPatterns, _section.TagPattern.Text))
            RefreshRecentPatterns();
        await _persistAsync();
    }
    ...
    private void RefreshPresets()
    {
        ...
        var presets = OptionsDefinitions.TagPresets
    ...
    private static bool IsPreset(string pattern) => OptionsDefinitions.TagPresets.Any(item =>
