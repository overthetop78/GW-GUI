...
public sealed record KeyboardChord(ModifierKeys Modifiers, IReadOnlyList<Key> Keys)
{
    ...
    public bool Matches(ModifierKeys modifiers, IReadOnlySet<Key> pressed) =>
        modifiers == Modifiers && Keys.Count == pressed.Count && Keys.All(pressed.Contains);

    public bool Contains(Key key) => Keys.Contains(key);
