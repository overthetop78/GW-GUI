...
internal sealed class RelativeMouseCapture
{
    ...
    internal void Capture(FrameworkElement display, FrameworkElement screen, IntPtr nativeHandle)
    {
        ...
        System.Windows.Input.Mouse.Capture(display);
        if (nativeHandle != IntPtr.Zero)
        {
            SetCapture(nativeHandle);
            HideNativeCursor();
        }
        Focus(display, nativeHandle);
        CenterCursor(screen);
    }

    internal static void Focus(FrameworkElement display, IntPtr nativeHandle)
    {
        if (nativeHandle != IntPtr.Zero) SetFocus(nativeHandle);
        else display.Focus();
    }
