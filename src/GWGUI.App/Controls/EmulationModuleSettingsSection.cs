...
internal sealed partial class EmulationModuleSettingsSection : UserControl
{
    ...
    internal EmulationModuleSettingsSection(IEmulationModule module)
    {
        ...
        if (module is IEmulationStorageSettingsManager storageManager)
            _storageSettings = new EmulationStorageSettingsController(storageManager);
    ...
    private UIElement BuildMachineTabs()
    {
        ...
        return EmulationMachineTabs.Create(tab => settings.Visibility.Tabs.GetValueOrDefault(tab)
            ...
            : null, $"{_module.Id}:{_configuration.MachineId}", TabActivatedAsync);
    }
    ...
    private UIElement BuildTab(EmulationMachineSettings settings, EmulationMachineTab tab)
    {
        ...
        if (tab == EmulationMachineTab.General && _emulatorManagement is not null)
            panel.Children.Add(_emulatorManagement.View);
    ...
    private static ComboBox CreateSelection(EmulationSettingsField field)
    {
    ...
    private static CheckBox CreateToggle(EmulationSettingsField field) => new()
    {
        IsChecked = field.Value == field.EnabledValue,
        Tag = field
    };
    ...
    private FrameworkElement CreatePath(EmulationSettingsField field)
    {
        ...
        browse.Click += (_, _) =>
        {
            var dialog = new OpenFileDialog { FileName = path.Text };
    ...
    private void RebuildEditor() => Content = BuildEditor();
    ...
    private Task TabActivatedAsync(EmulationMachineTab tab) =>
        tab == EmulationMachineTab.Rom && _firmwareManagement is not null
            ? _firmwareManagement.RefreshAsync()
            : Task.CompletedTask;
}
