...
internal sealed class MachineController : UserControl, IAsyncDisposable
{
    ...
    private void FullscreenHostSizeChanged(object sender, SizeChangedEventArgs args) => FitFullscreen();

    private void FitFullscreen()
    {
        if (_fullscreenHost is null) return;
        var frame = _session.Machine.Video.LatestFrame;
        var fitted = EmulationVideoLayout.Fit(_fullscreenHost.ActualWidth, _fullscreenHost.ActualHeight,
            frame?.AspectRatio ?? 4f / 3f);
    ...
    private void FramePresented(object? sender, VideoFrame frame)
    {
        ...
        foreach (var device in _options.MediaDevices)
        {
            ...
            if (activityStates.GetValueOrDefault(device.Slot))
                _mediaActivityUntil[device.Slot] = now + ControlTechnicalConstants.MediaActivityPersistence;
