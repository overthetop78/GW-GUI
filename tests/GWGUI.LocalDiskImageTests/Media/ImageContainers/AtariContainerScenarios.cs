...
internal static class AtariContainerScenarios
{
    public static void Rle(int damage)
    {
        ...
        Assert.Equal(packed, GWGUI.MediaEngine.Formats.Floppy.Msa.MsaRleEncoder.Pack(literal));
        Assert.Equal(literal, GWGUI.MediaEngine.Formats.Floppy.Msa.MsaRleDecoder.Unpack(packed, 8, 0, 0));
        ...
        Assert.Throws<InvalidDataException>(() => GWGUI.MediaEngine.Formats.Floppy.Msa.MsaRleDecoder.Unpack(invalid, 5, 0, 0));
    }
    public static async Task Atr(int size,int count,string format)
    {
        ...
        var writer = new GWGUI.MediaEngine.Formats.Floppy.Atr.AtrWriter(files);
    ...
    public static async Task Msa(bool compressed)
    {
        ...
        var files = new GWGUI.Tests.Application.TestInfrastructure.MemoryImageFiles();
        var writer = new GWGUI.MediaEngine.Formats.Floppy.Msa.MsaWriter(files); await writer.WriteAsync(image,"output");
        ...
        var reader = new GWGUI.MediaEngine.Formats.Floppy.Msa.MsaReader((_,token)=>{token.ThrowIfCancellationRequested();return Task.FromResult(written);});
    ...
    public static async Task Read(string extension,int length,int cylinders,int heads,int sectors,int blockSize,bool bpb=false)
    {
        ...
        await new GWGUI.MediaEngine.Formats.Floppy.St.AtariStWriter(linear).WriteAsync(image, outputPath);
