...
internal static class ExternalOutputParsingScenarios
{
    public static void Information(bool malformed)
    {
        ...
        var result=GWGUI.Infrastructure.Hardware.Parsing.GwInfoParser.Parse(text); Assert.Equal(text,result.RawOutput);
        ...
        var empty=GWGUI.Infrastructure.Hardware.Parsing.GwInfoParser.Parse(""); Assert.Null(empty.Model); Assert.Null(empty.Port); Assert.False(empty.HasNetworkWarning);
    }
